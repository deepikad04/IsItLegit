...
export default function PatternCard({ pattern }) {
  ...
  return (
    <div className={clsx(
      ...
      confidence >= 0.7 ? 'border-red-500' :
        confidence >= 0.5 ? 'border-amber-500' :
          'border-brand-blue'
    )}>
      <div className="flex items-center justify-between mb-2">
        <h4 className="font-bold text-brand-navy capitalize text-base">
          {(pattern.pattern_name || 'unknown pattern').replace(/_/g, ' ')}
        </h4>
        <div className="flex items-center gap-2">
            ...
            <div
              className={clsx(
                ...
                confidence >= 0.7 ? 'bg-red-500' :
                  confidence >= 0.5 ? 'bg-amber-500' : 'bg-brand-blue'
              )}
          ...
          <span className={clsx(
            ...
            confidence >= 0.7 ? 'bg-red-100 text-red-700' :
              confidence >= 0.5 ? 'bg-yellow-100 text-amber-700' :
